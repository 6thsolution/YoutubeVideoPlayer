...
class QueuePlayerLooper2 : NSObject, Looper2 {
    ...
    func start(in parentLayer: CALayer) {
        ...
        compositionTrack = composition.addMutableTrack(withMediaType: AVMediaTypeVideo, preferredTrackID: CMPersistentTrackID())
        currentTrack = videoAsset.tracks(withMediaType: AVMediaTypeVideo)[0]
        try! compositionTrack.insertTimeRange(assetTimeRange, of: currentTrack, at: CMTimeMake(0, 1))
        print("end get composition")
